...
def _open_image(image_source: BinaryIO | str) -> Image.Image:
    """Открывает и полностью загружает изображение, оборачивая ошибки Pillow."""
    try:
...
def open_image_from_file(file_obj: BinaryIO) -> Image.Image:
    """Читает изображение из файлового объекта с EXIF-поворотом и нормализацией."""
    file_obj.seek(0)
...
def make_image_variant_buffer(
    ...
    """Кодирует уменьшенную копию в буфер; возвращает буфер и итоговый размер."""
    img_copy = img.copy()
...
def make_webp_content(
    img: Image.Image, size: tuple[int, int], quality: int, suffix: str, base_name: str
) -> ContentFile:
    """Собирает WEBP-вариант как ContentFile с размерами в image_dimensions."""
...
def build_optimized_content(img: Image.Image, original_name: str) -> ContentFile:
    """Готовит оптимизированный вариант для полноэкранного просмотра."""
...
def build_thumbnail_content(img: Image.Image, original_name: str) -> ContentFile:
    """Готовит миниатюру для карточек галереи."""
    base_name = os.path.splitext(os.path.basename(original_name))[0]
